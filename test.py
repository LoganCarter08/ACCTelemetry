...
class AssettoCorsaData(object):
        def __init__(self):
            ...
            self.desiredFields = ["wheelSlipFL", "wheelSlipFR", "wheelSlipRL", "wheelSlipRR", "wheelPressureFL", "wheelPressureFR", "wheelPressureRL", "wheelPressureRR", "tc1", "abs1", "heading", "pitch", "roll", "TyreCoreTempFL", "TyreCoreTempFR", "TyreCoreTempRL", "TyreCoreTempRR", "brakeTempFL", "brakeTempFR", "brakeTempRL", "brakeTempRR", "brakeBias", "slipRatio1", "slipRatio2", "slipRatio3", "slipRatio4", "slipAngle1", "slipAngle2", "slipAngle3", "slipAngle4"]
        ...
        def getData(self):
            ...
            for index, value in enumerate(struct.unpack(self.layout, rawData)):
                if self.fields[index] in self.desiredFields:
        ...
        def _convertData(self, data):
            ...
            for newName in ['wheelSlip', 'wheelLoad', 'wheelsPressure',
                    ...
                    'suspensionTravel']:
                if newName in self.desiredFields:
                    data[newName] = []
...
if __name__ == '__main__':
    ...
    assettoReader.start()
    f = open("testDesiredOutput.json", "a")
    while True:
        f.write(str(assettoReader.getData()))
        f.write("\n")
        time.sleep(0.01)
